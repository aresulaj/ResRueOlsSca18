...
if ~isempty(detectFiles)
    ...
    for i = 1:length(detectTrode)
        ...
        % Detect spikes
        if detectChk
            [handles.expt spikes trodeInd] = DetectSpikes(handles.expt,detectFiles,i,thresh);
        else
            [handles.expt spikes trodeInd] = DetectSpikes(handles.expt,detectFiles,[],thresh);
        end
        
        if ~strcmp(handles.expt.expt_type,'vTrack')
...
if ~isempty(spikes)
    ...
    % spikes)
    if ~isfield(expt,'expt_type') || ~strcmp(expt.expt_type,'vTrack')
...
% --- Executes when entered data in editable cell(s) in SortTable.
function SortTable_CellEditCallback(hObject, eventdata, handles)
% hObject    handle to SortTable (see GCBO)
% eventdata  structure with the following fields (see UITABLE)
