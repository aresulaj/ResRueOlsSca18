...
UDP_OBJ_STIM_PC = udp(RHOST, 9094,'LocalPort', 9093,'Timeout',2);     % Address of stimulus PC
...
handles.aiparams.TimerPeriod =  0.1;   % Default value
...
if strcmp(RigDef.Daq.OnlinePlotting,'DataViewer');
    ...
    handles.aiparams.TriggerFcn = {RigDef.Daq.TriggerFcn,handles.hDataViewer};    % DataViewerCallback needs handle to DataViewer
    handles.aiparams.SamplesAcquiredFcn = {RigDef.Daq.SamplesAcquiredFcn,handles.hDataViewer};    % DataViewerCallback needs handle to DataViewer
end
...
addline(DIOBJ,8:15,'in','Condition');
...
p = handles.RigDef.Daq.Parameters;
DefineAIChannels(p);
...
daq.gain = handles.RigDef.Daq.Gain;
daq.Fs = handles.RigDef.Daq.SampleRate;
daq.filter = handles.RigDef.Daq.Filter;
set(AIOBJ,'UserData',daq);
...
AIOBJ.TimerPeriod = handles.aiparams.TimerPeriod;
...
if ~get(handles.tg_trkDaq,'Value')
    set(hDV.hAllAxes,'XLim',[0 handles.aiparams.sweeplength]);
else
    set(hDV.hAllAxes,'XLim',[0 handles.RigDef.trk.TimerPeriod]);
end
guidata(hObject,handles);
...
function tg_trkDaq_Callback(hObject, eventdata, handles)
rdef = RigDefs;
val = get(hObject,'Value');
if val
    set(hObject,'ForegroundColor',[0 0 1]);
    handles.aiparams.TimerFcn = '';%{rdef.trk.TimerFcn,handles.hDataViewer}; %AR ''
    handles.aiparams.TimerPeriod = rdef.trk.TimerPeriod; %AR
    handles.aiparams.TriggerFcn = {rdef.trk.TriggerFcn,handles.hDataViewer};
    handles.aiparams.SamplesAcquiredFcn ={rdef.trk.SamplesAcquiredFcn,handles.hDataViewer};%{rdef.trk.TimerFcn,handles.hDataViewer}; %AR '';
    hDV = guidata(handles.hDataViewer);
    set(hDV.hAllAxes,'XLim',[0 handles.RigDef.trk.TimerPeriod]);
    
    disp('**** DaqController set to track mode ****');
else
    set(hObject,'ForegroundColor',[0 0 0]);
    handles.aiparams.TimerFcn = '';
    handles.aiparams.TriggerFcn = {rdef.Daq.TriggerFcn,handles.hDataViewer};
    handles.aiparams.SamplesAcquiredFcn = {rdef.Daq.SamplesAcquiredFcn,handles.hDataViewer};
    hDV = guidata(handles.hDataViewer);
    set(hDV.hAllAxes,'XLim',[0 handles.aiparams.sweeplength]);
    
    disp('**** DaqController set to standard mode ****');
end
