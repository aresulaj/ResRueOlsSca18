function oriContrastFig(expt,unitTag,fileInd,b,saveTag)
...
% %     
%     contrastVal = [0.04 0.15 0.4 1];
%     offset = [0 12 24 36];
    

    contrastVal = [0.15 0.4 1];
    offset = [0 12 24];
    
    for m = 1:length(stim.values)
        for n = 1:length(contrastVal)
            stim.code{m,n} = m+offset(n);
        end
    end
...
%         cond.values = {1 2 3 4};
    ...
    for m = 1:length(stim.values)
        for n = 1:length(contrastVal)
            cspikes(m,n) = filtspikes(spikes,0,'stimcond',stim.code{m,n});
        end
    end
...
%     h.nori.l = plotOrientTuning(nallfr.ledon,theta,h.nori.ax);
    ...
    for i = 1:length(cond.values)
        if strcmp(cond.type,'led')
            ...
            wnames = fieldnames(w);
        elseif strcmp(cond.type,'contrast')
            tempspikes = filtspikes(spikes,0,'stimcond',cell2mat(stim.code(:,i)));
            wnames = fieldnames(w);
        else
