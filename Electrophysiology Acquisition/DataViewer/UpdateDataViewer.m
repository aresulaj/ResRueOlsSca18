...
if isfield(handles,'ExptDataViewer')
    margins = [0.05 0.035 0.042 0.035]; %[0.05 0.035 0.042 0.08];
else
    margins = [0.05 0.035 0.042 0.035];
end
if(nPlotOn ~= 0)
    if(showAllYAxes)
        interAxisSpace = 0; % WB fixit remove hardcode (right now must keep in sync w/ InitializeDataViewerPlots.m)
