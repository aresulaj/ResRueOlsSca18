...
if(nPlotOn ~= 0)
    if(showAllYAxes)
        interAxisSpace = 0; % WB fixit remove hardcode (right now must keep in sync w/ DataViewerUpdate.m)
...
ChannelOrder =[1 22 28 18 25 23 29 19 27 26 31 21 20 32 24 30 2:17]; %AR added this
...
for i = 1:nPlotOn
    ...
    hPlotLines(k) = line([0 handles.SweepDuration],[-10 10],'Parent',hAxisTemp,'Color',altColor(mod(i,2)+1,:));
