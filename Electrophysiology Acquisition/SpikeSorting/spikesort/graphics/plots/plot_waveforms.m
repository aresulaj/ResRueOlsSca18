...
%                  - default = 0
   ...
   if nargin < 2, show = 1:size(spikes.waveforms,1); end
    ...
    % --- Modification for scaling each --- %
    data.ylims = [max(min(data.waveforms(:)), mean(data.waveforms(:)) - 4*std(data.waveforms(:))) ...
        min(max(data.waveforms(:)), mean(data.waveforms(:)) + 4*std(data.waveforms(:)))];
    % --- Modification for scaling each --- %
